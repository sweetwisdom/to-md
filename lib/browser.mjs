...
const STEALTH_VIEWPORT = { width: 1200, height: 800 };
...
export async function fetchPage(url, options = {}) {
  ...
  try {
    ...
    // Assign unique alt to every img (Defuddle _deduplicateImages removes duplicates by alt)
    await page.evaluate(() => {
      const seen = new Map();
    ...
    // Inject DOMPurify and sanitize body
    await page.addScriptTag({ url: DOMPURIFY_CDN });
    const bodyHtml = await page.evaluate((config) => {
      return DOMPurify.sanitize(document.body.innerHTML, config);
    }, DOMPURIFY_CONFIG);
